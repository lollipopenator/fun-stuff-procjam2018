...
var utilities = {
	createThree: function(holder, init, update, postRender, preventRender) {
		...
		init(time, scene, camera)
		animate();

	},

	createThree3: function(holder, init, update, postRender, preventRender) {
		...
		//var orbitalCamera = new THREE.StereoCamera(55, w / h, 1, 10000);
		var orbitalCamera = new THREE.PerspectiveCamera(55, w / h, 1, 10000);
		//var orbitalCamera = new THREE.OrthographicCamera( w / - 2, w / 2, h / 2, h / - 2, 1, 1000 );
		...
		function render() {
			...
			update(time, scene, orbitalCamera, renderer.context);
			//camera.lookAt(scene.position);
			renderer.render(scene, orbitalCamera);
			if (postRender)
				postRender(time, scene, orbitalCamera, renderer.context);
		}


		//let pos = new THREE.Vector3()
		orbitalCamera.updateScreenPos = function(worldPos, screenPos) {
			// Clone the worldPos and set this to it


			screenPos.set(worldPos.x, worldPos.y, worldPos.z);

			//let scale = heightHalf / widthHalf;
			let scale = 1;


			screenPos.project(orbitalCamera);
			screenPos.x = scale * (screenPos.x * w / 2);
			screenPos.y = -scale * (screenPos.y * h / 2);
		};

		init(time, scene, orbitalCamera);
...
extend(Vector.prototype, {
	...
	polarOffsetBezierWithPolarControlPoints: function(g, p, rA, thetaA, rB, thetaB, r0, theta0, r1, theta1) {
		...
		// g.vertex(p.x + r0 * Math.cos(theta1) + rA * Math.cos(thetaA),
		// 		p.y + r0 * Math.sin(theta1) + rA * Math.sin(thetaA));

		// 	g.vertex(this.x + r1 * Math.cos(theta1) + rB * Math.cos(thetaB),
		// 		this.y + r1 * Math.sin(theta1) + rB * Math.sin(thetaB));

		// 	g.vertex(this.x + rB * Math.cos(thetaB),
